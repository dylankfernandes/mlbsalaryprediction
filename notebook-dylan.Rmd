...
# Normalizing by Year
```{r}
salary_by_year <- slries %>% group_by(yearID) %>% summarise_at(vars(salary), mean)
salary_by_year$yearID <- salary_by_year$yearID - 1984

salary_regressor = lm(formula = salary~yearID, data = salary_by_year)
...
normalize_year <- function(salary, yearID) {
  # (salary - intercept)/(slope) * yearID
  # year_salary <- salary_by_year %>% filter(yearID == 16)
  # salary - year_salary$salary
  salary
}

df <- df %>% mutate(salary = normalize_year(salary, yearID))
df
```
