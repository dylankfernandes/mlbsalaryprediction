...
```{r}
awards_asg_batting_app <- awards_asg_batting %>% left_join(Appearances)
```
...
```
```{r}
test_set$predicted.y = predict(brnn, newdata = test_set)
cor(test_set$adjustedSalary,test_set$predicted.y)
act_pred <- data.frame(cbind(actuals =test_set$adjustedSalary, predicteds= test_set$predicted.y ))
head(test_set)
df_with_player_id$ID= df_with_player_id$playerID
df_with_player_id
test = df_with_player_id[which(df_with_player_id$yearID == 2016),]
traviswood = test[which(test$playerID == "woodtr01"),]
traviswood
testresult = test_set[which(test_set$adjustedSalary == 6170000),]

```
```{r}
ranger <- train(adjustedSalary ~ ., data = training_set, 
                 method = "ranger",
                 ## This last option is actually one
                 ## for gbm() that passes through
                 verbose = FALSE)
...
test2.features = subset(test2, select=-c(adjustedSalary))
test2.target = subset(test2, select=adjustedSalary)[,1]
predictions = predict(lm, newdata = test2.features)

# RMSE
cor(test2.target, predictions)^2
